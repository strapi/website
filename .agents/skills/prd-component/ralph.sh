...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  if (( asd_rc == 2 )); then
    echo "Error: PRD parse error in all_stories_done. Restoring backup."
...
TASK_PROMPT
  ...
  if ! jq -e '.' "$PRD_FILE" >/dev/null 2>&1; then
    echo "Error: PRD corrupted after Claude call. Restoring backup."
    cp "${PRD_FILE}.bak" "$PRD_FILE"
...
all_stories_done || asd_rc=$?
if (( asd_rc == 2 )); then
  echo "Error: PRD parse error after loop. Cannot determine final status."
  exit 1
fi
if (( asd_rc == 0 )); then
  run_final_review
